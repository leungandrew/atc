...
func (manager CredhubManager) NewVariablesFactory(logger lager.Logger) (creds.VariablesFactory, error) {
	...
	_, err := a.Target(manager.URL, manager.caCerts, manager.Insecure)
	if err != nil {
		return nil, err
	}

	// FIXME Switch to client credentials
	_, err = a.LoginWithPassword("credhub", "password")
	if err != nil {
		return nil, err
	}

	return NewCredhubFactory(logger, a, manager.PathPrefix), nil
}
